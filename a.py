...
a=st.sidebar.radio('Navigation',['Welcome','Show Database','Show Visualization','Manual ML Module','Automated ML Module'])
...
if a=='Show Database':
    ...
    # we created three checkbox for first navigation option and if its true showing database as per requirement.
        ...
        if st.checkbox("BAR CHART"):
            ...
            st.line_chart(data_complete['windspeed'])
            st.line_chart(data_complete['Temperatu re'])
            st.line_chart(data_complete['Humidity'])
            ...
            st.line_chart(data_complete['windspeed'])
            st.line_chart(data_complete['Temperature'])
            st.line_chart(data_complete['Humidity'])
    ...
    st.header("'FOREST FIRE PREDICTION USING MACHINE LEARNING LINEAR REGRESSION ALGORITHM' ")
    st.header("Project by-")
    st.subheader("Akash Nilkanth")
    st.subheader("Siddesh Karpe")
    st.subheader("Mayur Gavali")
    st.subheader("Tushar Bhamare")
